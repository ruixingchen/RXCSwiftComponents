//
//  ASTableNode+RXCScrollViewProtocol.swift
...
extension ASTableNode: RXCScrollViewProtocol {
    ...
    public var rsc_delegate: UIScrollViewDelegate? {
        ...
        set {
            if let valid = newValue {
                precondition(valid is ASTableDelegate, "ASTableNode要求delegate必须同时遵循ASTableDelegate")
                self.delegate = (valid as! ASTableDelegate)
            }else {
                self.delegate = nil
            }
        }
